			...
			public void windowOpened(WindowEvent e) {
				try {
					...
					if (lecturerComboBox.getItemCount() == 0) {
						JOptionPane.showMessageDialog(getContentPane(), "На данной кафедре нет других преподавателей!",
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(ItemEvent -> {
					...
					try {
						...
						/*
						 * FIXME: а если не поменяли (ошибка) то как второй
						 * форме узнать об этом (т.е. значит ей нельзя удалять
						 * или я удаляю здесь
						 */
						dispose();
					}
