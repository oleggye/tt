...
public class ValidationServiceImpl implements IValidationService {
	...
	@Override
	public void validateClassroom(Classroom classroom, boolean withId) throws ServiceValidationException {
		StringBuilder builder = new StringBuilder();
	...
	private static class Checker {
		...
		static boolean isWeekNumberInvalid(byte weekNumber) {
			if (weekNumber < LOW_WEEK_NUMBER_BOUD || weekNumber > TOP_WEEK_NUMBER_BOUD)
				return true;
			else
				return false;
		}
		...
		static boolean isWeekDayInvalid(byte weekDay) {
			if (weekDay < LOW_WEEK_DAY_BOUD || weekDay > TOP_WEEK_DAY_BOUD)
				return true;
			else
				return false;
		}
		...
		static boolean isSubjectOrdinalNumberInvalid(byte subjOrdinalNumber) {
			if (subjOrdinalNumber < LOW_SUBJECT_ORDINAl_NUMBER_BOUD
